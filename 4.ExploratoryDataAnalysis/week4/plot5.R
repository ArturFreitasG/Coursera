...
# filtering Baltimore data and "on-road" type. Agreggating by year
baltimore_by_year <- NEI %>%
                     filter(fips == "24510", type == c("ON-ROAD")) %>%
                     group_by(year) %>% 
...
with(baltimore_by_year, barplot(height = total_emissions,
                                ...
                                main="Total PM2.5 emissions in Baltimore per year (motor vehicle sources)"))

# saving plot 5 into .png file
dev.copy(png,paste0(week4_dir, "/plot5.png"))
dev.off()